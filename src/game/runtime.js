...
(function () {
  ...
  // A horizontal swipe changes exactly one lane; a second finger still engages boost.
  const touch = { id: null, x0: 0, y0: 0, dx: 0, dy: 0, boost: false, boostId: null };
  ...
  const PREVIEW_T = 3.2, PREVIEW_BLEND = 0.7, TITLE_CARD_T = 2.0;
  ...
  function fingerDown(id, x, y) {
    ...
    if (touch.id === null) { touch.id = id; touch.x0 = x; touch.y0 = y; touch.dx = 0; touch.dy = 0; }
  ...
  function fingerUp(id) {
    ...
    if (id === touch.id) {
      ...
      touch.id = null; touch.dx = 0; touch.dy = 0;
    }
  ...
  function update(dt) {
    ...
    // Stable runner control: all player motion resolves to one of three backbone-local lanes.
    const laneEngine = window.foldSpacerLaneEngine;
    if (!P.done && laneEngine) {
      ...
      P.x += (0 - P.x) * Math.min(1, 12 * dt);
      P.y += (0 - P.y) * Math.min(1, 12 * dt);
      P.vx = 0; P.vy = 0;
    }
    // corridor
    const node = rail.nodeAt(P.s);
  ...
  function draw() {
    ...
    if (titleCardT > 0 && fold && !P.done) {
      const a = Math.min(1, titleCardT / 0.6) * Math.min(1, (TITLE_CARD_T - titleCardT) / 0.4 + 0.01);
