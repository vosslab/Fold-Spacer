...
(function () {
  ...
  window.addEventListener('keydown', (e) => { skipPreview();
    ensureDrone();
    if (e.key === 'a' || e.key === 'A') { if (!e.repeat) autopilot = !autopilot; e.preventDefault(); return; }
    if (ARROWS[e.key]) {
      // Double-tap a side to roll that way. Not gated on free flight: in lane mode an arrow already
      // lunges, and a double-tap lunging AND rolling is exactly how the touch flick layers.
      if (!e.repeat) {
        const d = ARROWS[e.key], now = performance.now();
        if ((d === 'left' || d === 'right') && tapLast.d === d && now - tapLast.t < 320) { startRoll(d === 'right' ? 1 : -1); tapLast.t = 0; }
        else { tapLast.d = d; tapLast.t = now; }
      }
      if (laneMode) { if (!e.repeat) laneSwipe(ARROWS[e.key]); } else { keys[ARROWS[e.key]] = true; autopilot = false; } e.preventDefault(); }
    if (e.key === 'Shift' || e.key === ' ') { keys.boost = true; e.preventDefault(); }
    if (e.key === 'Control') keys.brake = true;
    if (e.repeat) return;
    if (e.key === 'r' || e.key === 'R') reset();
    if (e.key === 'l' || e.key === 'L') { laneMode = !laneMode; flash = { text: laneMode ? 'lunge mode · tap an arrow to lunge that way, the craft snaps back' : 'free flight', t: 2.5 }; }
  ...
  const KEY_W = 15;          // rad/s of the keyboard's steering spring, as LUNGE.wFollow is for a finger
  const KEY_BACK = 2.0;      // how fast a key ramps its axis out to the ring, and back on release
  ...
  function fingerDown(id, x, y) {
    ...
    if (touch.id === null) { touch.id = id; touch.x0 = x; touch.y0 = y; touch.dx = 0; touch.dy = 0; touch.t0 = performance.now(); touch.hold = false; touch.swiped = false; if (!laneMode) autopilot = false; }
  ...
  function update(dt) {
    ...
    const hands = keys.left || keys.right || keys.up || keys.down || stick;
    if (laneMode && !autopilot && !P.done) {
      ...
      if (window.TRACE_LUNGE) (status.traj = status.traj || []).push([+P.t.toFixed(2), +(P.x / SC).toFixed(2), +(P.y / SC).toFixed(2), lunge.active ? 1 : 0, +lunge.t.toFixed(2), +(rho / SC).toFixed(2), +(tx / SC).toFixed(2)]);
      ...
      P.vy += clamp((vdy - P.vy) / dt, -acc, acc) * dt;
      status.oracle = true;
    } else if (hands && !autopilot) {
      // The keyboard used to add raw acceleration in four directions and nothing else: no target, no
      // return, no proportionality. The phone has a critically damped spring toward a POSITION, and that
      // is the whole reason it feels decisive — press and the craft goes somewhere definite, let go and
      // it comes back. The keys now drive the same spring, toward the same ring the finger reaches, with
      // the acceleration clamped to LAT_ACC exactly as the lunge clamps it, so the reach the fairness
      // model is built on is unchanged.
      ...
      // a key ramps its axis out to the ring and back, so a tap is a small definite move and a hold
      // carries you to the edge and keeps you there
      const kx = (keys.right ? 1 : 0) - (keys.left ? 1 : 0);
      const ky = (keys.up ? 1 : 0) - (keys.down ? 1 : 0);
      const ramp = (cur, want) => cur + clamp(want - cur, -KEY_BACK * dt, KEY_BACK * dt);
      P.keyX = ramp(P.keyX || 0, kx); P.keyY = ramp(P.keyY || 0, ky);
      let tx, ty;
      ...
      } else { tx = P.keyX * ringKX; ty = P.keyY * ringKY; }
      ...
      for (let i = 0; i < stepsK; i++) {
        let ax = wK * wK * (tx - P.x) - 2 * wK * P.vx;
        ax = clamp(ax, -accK, accK);
