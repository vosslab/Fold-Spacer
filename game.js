...
(function () {
  ...
  // Side chains are physical hazards. Their real atom surfaces and bonds define a collision.
  ...
  function reset() {
    ...
    for (const b of blocks) { if (b.type === 'H') { b.f = 0; b.anim = false; b.judged = false; b.collided = false; b.minD = Infinity; b.side = 0; b.perfect = false; b.pendingPerfect = false; } b.passed = false; }
  ...
  // The runner keeps the protein's own moving cross-section, but offers only three stable tracks in it.
  // Each obstacle remains the residue's real ball-and-stick side chain; only its deliberately displaced
  // pose is aimed through one of the three tracks. If its true length cannot reach any track, omit it.
  ...
  function arrangeLaneObstacles() {
    for (let k = blocks.length - 1; k >= 0; k--) {
      ...
      for (const lane of order) {
        ...
        const cap = 1.05 * b.Lvis;
        if (V.len(offset) < 1e-6 || need + 0.25 > cap) continue;
        b.lane = lane;
        b.inw = V.norm(offset);
        b.inLen = Math.min(cap, Math.max(0.8, need + 0.35));
  ...
  function update(dt) {
    ...
    if (P.preview > 0) {
      ...
      if (P.preview < PREVIEW_BLEND) {          // ease into exactly where the flight camera starts
        ...
        cam.fwd = V.norm(V.lerp(pv.fwd, fs…2284 tokens truncated…easures whether the game is too hard.
